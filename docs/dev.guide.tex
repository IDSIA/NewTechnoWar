...
\begin{document}
	...
	\begin{figure}[ht!]
		...
		\begin{tikzpicture}[
			...
			\node[state,label=right:\texttt{\_goRound()}]		(Round)   	[below of=Init] 	{};
			\node[state,label=below:\texttt{\_goCheck()}]     	(Check)  	[below of=Round] 	{};
			\node[state,label=below:\texttt{\_goResponse()}]  	(Response)	[right of=Check] 	{};
			\node[state,label=below:\texttt{\_goUpdate()}]    	(Update)	[left of=Check] 	{};
			\node[state,accepting,label=right:\texttt{\_goEnd()}]			(End)		[below of=Check]  	{};

			\path[->]
			(Init)  	edge node				{} (Round)
